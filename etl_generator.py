...
def generate_etl_script(mappings_json_path: str, source_file_path: str, output_script_path: str):
    """Generates a Python ETL script based on a mapping JSON file."""

    # The template for the transformation script
    script_template = f"""
import pandas as pd
import json
import os
...
def main():
    # --- Configuration ---
    SOURCE_FILE = r'{os.path.abspath(source_file_path)}'
    MAPPINGS_FILE = r'{os.path.abspath(mappings_json_path)}'
...
"""
    try:
        ...
        print(f"❌ Error writing ETL script to '{output_script_path}': {e}")